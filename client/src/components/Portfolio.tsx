...
export function Portfolio() {
  ...
  const [selectedRepoModal, setSelectedRepoModal] = useState<string | null>(null);
  ...
  useEffect(() => {
    // Enable offline persistence in firebase.ts handles the caching.
    // We just listen normally.
    ...
    const unsubscribe = onSnapshot(q, 
      ...
      (error) => {
        ...
        setLoading(false);
        // If we are offline, we might get an error or just stale data.
        // With persistence, it should be fine.
        if (error.code === 'permission-denied') {
           ...
           // This often happens when offline, but with persistence it should be masked.
  ...
  return (
    <Section id="portfolio" className="bg-gradient-to-b from-white/3 via-white/2 to-transparent rounded-3xl my-20">
      ...
      {/* REPOSITORY MODAL - Full Screen View */}
      <Dialog open={!!selectedRepoModal} onOpenChange={(open) => {
        if (!open) {
        ...
        <DialogContent className="!fixed !inset-0 !left-auto !top-auto !translate-x-0 !translate-y-0 !max-w-none !w-screen !h-screen !p-0 !flex !flex-col !bg-background !border-0 !shadow-none !rounded-none !z-[200] !m-0 !gap-0">
          {activeRepo && (
            <div className="flex flex-col h-full w-full overflow-hidden">
              {/* Header */}
              <div className="shrink-0 bg-gradient-to-b from-background/80 to-transparent p-6 z-10">
                <button 
                  onClick={() => {
                    setSelectedRepoModal(null);
                    setCurrentView("repositories");
                ...
                <h2 className="text-3xl font-display font-bold mb-2">{activeRepo.title}</h2>
                <p className="text-muted-foreground">{activeRepo.description}</p>
              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
                  ...
                  {isEditMode && (
                    <Dialog open={newProjectOpen} onOpenChange={setNewProjectOpen}>
                      ...
                      <DialogContent className="sm:max-w-[425px]">
                        <DialogTitle>Add Project to {activeRepo.title}</DialogTitle>
                        <div className="grid gap-4 py-4">
                          <div className="grid gap-2">
                          ...
                          <div className="grid gap-2">
                              ...
                              <Input 
                                  ...
                                  if (files.length > 0) {
                                      ...
                                      const uploadPromises = files.slice(0, 5).map(file => 
                                          uploadImageToStorage(file, `projects/${activeRepo.id}/${Date.now()}_${file.name}`)
                                      );
                            ...
                            <div className="flex gap-2 overflow-x-auto py-2">
                              {newProject.images.map((img, i) => (
                                <div key={i} className="relative shrink-0 h-16 w-16">
                                   <img 
                                      src={img} 
                                   ...
                                   />
                                   <button onClick={() => setNewProject(prev => ({...prev, images: prev.images.filter((_, idx) => idx !== i)}))} className="absolute -top-1 -right-1 bg-red-500 text-white rounded-full p-0.5">
                                     <X className="w-3 h-3" />
                                   </button>
                                </div>
                              ))}
